		...
		socket.on('chat_attachment', (data) => {
			if (data.groupId) {
				...
				if (clients[data.groupId]) {
					emitNotifyToArray(clients, data.groupId, io, 'response_chat_attachment', response)
				}
			}
			if (data.contactId) {
				let response = {
				...
				if (clients[data.contactId]) {
					emitNotifyToArray(clients, data.contactId, io, 'response_chat_attachment', response)
				}
			}
